...
if (isset($_SESSION['uname'])) {
    ...
    $dat = date('y-m-d');
    $dat=$y.$dat;
    $query = "SELECT * from users,relation_tab,site WHERE users.Customer_ID = relation_tab.Customer_ID AND relation_tab.Site_ID = site.site_ID AND (site.Hand_over_date > '$dat' OR site.Hand_over_date is NULL)";
...
<body>
    ...
    <table align="center" border="1px" style="width:800px;background-color: whitesmoke; line-height:40px; text-align: center; padding:20px;margin:auto;">
        <tr>
            <th colspan="30">
                <h2>All Active customer Records</h2>
            </th>
        </tr>
        <th> ID </th>
        <th> Client Name </th>
