...
if (isset($_SESSION['uname'])) {
    ...
    $dat = date('y-m-d');
    $dat = $y . $dat;
    $query = "SELECT * from site where Hand_over_date>'$dat' OR Hand_over_date is NULL ";
    ...
    <table align="center" border="1px" style="width:800px;background-color: whitesmoke; line-height:40px; text-align: center; padding:20px;margin:auto;">
        ...
        ?>
            <tr>
                ...
                <td><?php echo $rows['Construction_grade']; ?></td>
                <td><?php echo $rows['Emp_ID']; ?></td>
                <td><?php echo $rows['Site_address']; ?></td>
                <td><?php echo $rows['Start_date']; ?></td>
                <td><?php echo $rows['Promise_date']; ?></td>
                <td><?php
                    if ($rows['Hand_over_date'] == "") {
                        echo "N/A";
                    } else {
                        echo $rows['Hand_over_date'];
                    }
                    ?></td>
                <td><?php echo $rows['Site_stat']; ?></td>
            </tr>
        <?php
        }
        ?>
